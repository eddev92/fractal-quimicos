...
"use strict";
	...
	$('a.aHome[href^=#], nav a[href^=#], a.top[href^=#], a.read[href^=#]').click(function(event) {
			...
			var result = ratio * NavHeight;

			$('html,body').animate({
				scrollTop: $(this.hash).offset().top - result
			}, 1000);
	...
	$("#contact_form").submit(function() {
			  ...
			  var sugerencia = $("#sugerencia").val();
					if (sugerencia == "") {
					$('#sugerencia').addClass('reqfld');
					$('<span class="error" style="display:none;">Required!</span>').insertBefore('#sugerencia').fadeIn(400);
					$("#sugerencia").focus(function() {  $('#sugerencia').removeClass('reqfld');  $(this).prev().fadeOut(400);});
					return false;
			  }
			  ...
			  var dataString = 'name='+ name + '&email=' + email + '&phone=' + phone + '&sugerencia=' + sugerencia + '&comment=' + comment + '&security=' + security;
